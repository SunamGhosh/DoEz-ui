...
import { Menu, X, LogOut, ChevronRight, Phone, Mail, MapPin, Globe } from "lucide-react";
...
const Navbar = () => {
  ...
  return (
    <>
      ...
      {/* Main Navbar */}
      <nav
        className={`sticky top-0 left-0 right-0 z-40 bg-white transition-all duration-300 ${isScrolled ? "shadow-md py-3" : "border-b border-gray-100 py-5"
          }`}
      >
            ...
            <div className="hidden md:flex items-center space-x-1">
              {navLinks.map((link) => (
                <Link
                  key={link.name}
                  to={link.href}
                  className={`px-5 py-2.5 rounded-full text-sm font-bold transition-all duration-300 ${location.pathname === link.href
        ...
        <div
          className={`md:hidden fixed inset-0 z-50 bg-white transition-all duration-300 ease-in-out ${isMobileMenuOpen ? "opacity-100 translate-x-0" : "opacity-0 translate-x-full pointer-events-none"
            }`}
        >
                ...
                <div>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-3 text-gray-600 font-medium">
                      <div className="w-8 h-8 rounded-full bg-teal-50 flex items-center justify-center text-teal-600"><Phone size={16} /></div>
                      +1 (555) 123-4567
                    </div>
                    <div className="flex items-center gap-3 text-gray-600 font-medium">
                      <div className="w-8 h-8 rounded-full bg-teal-50 flex items-center justify-center text-teal-600"><Mail size={16} /></div>
                      support@doez.com
                    </div>
                  </div>
                </div>
